...
const name = 'clearqueen';
const mod = 'Queen';
const command = `${commandPrefix}${name}`;
const usage = `\`${command} [queenname]\``;
const description = "Clears the image data of a given queen.";
...
function execute(msg) {
    ...
    if (args) {
        jsonfile.readFile(pathToQueen, (err, obj) => {
            ...
            msg.channel.send(`${success} Cleared image data for **${titleCase(args)}**.`).then((message) => {
                logger.info(`${command}: Cleared image data for ${args}`);
                return;
            });
        });
